...
function gradeHtml(grade, expanded = false) {
  ...
  return `<div class="form-grade"><span class="grade-letter ${grade.letter ? '' : 'no-grade'}">${grade.letter ?? '—'}</span>
    <div><h3>${grade.letter ? 'Personal form' : 'Insufficient data'}</h3><p class="fact">Personal form · compared with your other ${grade.n} same-mode games</p></div></div>
...
function goalEvidenceHtml(result, teaching = false) {
  ...
  if (result.status === 'mode-mismatch') return `<p class="fact">Goal not applied — this commitment is for ${esc(matchModeLabel(result.goal.matchMode))} matches.</p>`;
...
function coachingCardHtml(model, state) {
  ...
  const latest = state.items.find((it) => !['none', 'mode-mismatch'].includes(matchGoalResult(it, c).status));
  const learned = c.learned.length ? `<p class="fact">Learned goals: ${c.learned.map((g) => `${esc(g.label)}${g.matchMode == null ? '' : ` <span class="faint">(${esc(matchModeLabel(g.matchMode))})</span>`}`).join(' · ')}</p>` : '';
  ...
  return `<section class="card goal-card"><div class="card-head"><h2 class="card-label accent">${active ? 'Your focus' : 'Choose your next focus'}</h2><span class="faint">One goal. Automatic feedback.</span></div>
    ${active ? `<p class="goal-title">${esc(active.label)}</p><p class="fact">${active.matchMode == null ? 'All modes' : esc(matchModeLabel(active.matchMode))} · hit ${progress.hits} of ${progress.measured} measured, committed games in this window. Learned at 8 of 10.</p>
    ...
    <p class="faint">${model.openSession ? 'Session running. Refresh after a matching-mode game to see its result.' : 'Start a session before you queue to commit this target.'}</p>` : `<p class="lede">Commit before you queue. Vantage checks the result after each ${esc(matchModeLabel(model.analysisMode)).toLowerCase()} match.</p><div class="suggestions">${model.suggestions.map((g, i) => `<div class="suggestion"><div><span class="goal-label">${esc(g.label)}</span><span class="faint">${esc(matchModeLabel(g.matchMode))} · ${g.source === 'wins' ? `based on ${g.nWins} wins in this window` : 'starting target · not enough separating evidence yet'}</span></div><button class="btn-primary" data-action="adopt" data-index="${i}">Start session</button></div>`).join('') || '<p class="fact">You have learned the available suggested goals. Their recent results are still checked below.</p>'}</div>`}
    ${latestHtml}${learned}${regression}<p class="faint">${model.withoutSession.without} of your last ${model.withoutSession.total} applicable games had no recorded session goal.</p></section>`;
}

function matchWindowHtml(state) {
  if (!state.items.length) return '<section class="card"><h2>No match reviews available yet</h2><p class="fact">Refresh to retry missing match data. Up to 10 recent games will appear here.</p></section>';
  ...
  const tiles = [...state.items].reverse().map((it) => {
    const comparisonProfiles = state.items.filter((other) => sameMatchMode(other, it)).map((other) => other.profile);
    const grade = gradePersonalForm(it.profile, comparisonProfiles);
    ...
    return `<button class="match-tile ${sel ? 'selected' : ''}" data-action="select" data-match-id="${it.entry.match_id}" aria-pressed="${sel}" aria-label="${esc(hero)}, ${result}, ${esc(matchModeLabel(it.entry.match_mode))}, ${grade.letter ? `personal form ${grade.letter}` : 'insufficient data'}, ${esc(relativeTime(it.entry.start_time))}">${icon ? `<img src="${esc(icon)}" alt="">` : ''}<strong>${grade.letter ?? '—'}</strong><span class="tile-hero">${esc(hero)}</span><span class="tile-mode">${esc(matchModeLabel(it.entry.match_mode))}</span><span class="${it.profile.win ? 'up' : 'down'}">${result}</span></button>`;
  }).join('');
  const { entry, profile } = selected;
  const sameModeProfiles = state.items.filter((it) => sameMatchMode(it, selected)).map((it) => it.profile);
  const others = sameModeProfiles.filter((p) => p.matchId !== profile.matchId);
  ...
  return `<section class="card window-card"><div class="card-head"><h2 class="card-label">Your last ${state.items.length} games</h2><span class="faint">${wins} wins · ${state.items.length - wins} losses</span></div><p class="faint">Oldest → newest · select a match</p><div class="match-tiles">${tiles}</div>
    <div class="detail"><div><div class="detail-hero"><div><h3 class="detail-name">${esc(heroName(entry.hero_id))}</h3><span>${esc(matchModeLabel(entry.match_mode))} · ${profile.win ? 'Win' : 'Loss'} · ${duration(profile.durationS)} · ${relativeTime(entry.start_time)}</span></div></div>${gradeHtml(gradePersonalForm(profile, sameModeProfiles))}${goalEvidenceHtml(matchGoalResult(selected, state.coaching), state.teaching)}</div>
...
function renderExperience(container, state) {
  ...
  const selected = state.items.find((it) => it.entry.match_id === state.selectedMatchId) ?? state.items[0];
  const analysisMode = selected?.entry.match_mode;
  const modeItems = state.items.filter((it) => it.entry.match_mode === analysisMode);
  const sep = separators(modeItems.map((it) => it.profile));
  const learnedMetrics = new Set(state.coaching.learned.filter((g) => g.matchMode == null || g.matchMode === analysisMode).map((g) => g.metric));
  const applicableItems = state.coaching.active ? state.items.filter((it) => goalAppliesToItem(state.coaching.active, it)) : state.items;
  const model = { sep, split: splitRecord(modeItems.map((it) => it.profile), (p) => p.soulsVsLobby10, 0),
    analysisMode, analysisCount: modeItems.length,
    suggestions: suggestGoals(sep, Date.now(), 8).map((g) => ({ ...g, matchMode: analysisMode })).filter((g) => !learnedMetrics.has(g.metric)).slice(0, 3),
    openSession: state.coaching.sessions.at(-1)?.endedAt === null ? state.coaching.sessions.at(-1) : null,
    withoutSession: { total: applicableItems.length, without: applicableItems.filter((it) => ['none', 'unavailable'].includes(matchGoalResult(it, state.coaching).status)).length },
